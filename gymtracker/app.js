  ...
  app.innerHTML = `
    <div class="screen day-screen">
      <header class="page-header">
        ...
        <div>
          <div class="page-emoji">${day.emoji}</div>
...
async function renderExercise(dayId, exerciseId) {
  ...
  app.innerHTML = `
    <div class="screen exercise-screen">
      <header class="page-header">
        <button class="back-btn" onclick="navigate('day', ${dayId})">← Back</button>
        <div>
          <h2 class="page-title">${ex.name}</h2>
          <div class="ex-target">${ex.sets} × ${ex.reps} Wdh.</div>
        </div>
        <button class="history-btn" onclick="navigate('history', '${exerciseId}')">📈</button>
      </header>
...
function renderSetRow(index, set, lastSet) {
  return `
    <div class="set-row" id="set-${index}">
      ...
      <button class="set-done-btn" id="done-${index}" onclick="markSetDone(${index})">✓</button>
    </div>
  `;
}
...
function addSet(exerciseId, dayId) {
  ...
  if (container) {
    container.innerHTML = sets.map((s, i) => renderSetRow(i, s, null)).join('');
  }
}
...
function timerDoneAnimation() {
  ...
  const label = document.getElementById('timer-label');
  if (label) label.textContent = '✅ Los geht\'s!';
  setTimeout(() => {
